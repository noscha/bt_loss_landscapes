...
def grid_traversal(model, stepwidth, size_volume, accuracy=3, limit=10_000_000):
    iterations = 0
    ...
    while queue and iterations < limit:
        ...
        element_to_pop = 0  # random.randrange(len(queue))
        curr_coord = queue.pop(element_to_pop)

        for coord in neighboring_coords(curr_coord, stepwidth, accuracy):

            if coord not in visited and model.evaluate_loss(coord) <= c:
                queue.append(coord)
                visited.add(coord)
